...
class Predictor(BasePredictor):
    ...
    async def _run_warmup_test(self, weights: CogPath) -> None:
        ...
        generator = self.predict(
            **(
                ...
                | {
                    ...
                    "audio": [weights.resolve().joinpath("multilingual_sample.wav")],
    ...
    async def setup(self, weights: CogPath | str | None) -> None:
        ...
        # Run warmup test
        await self._run_warmup_test(weights)
    ...
    async def predict(
        ...
        audio: list[CogPath] | None = Input(
            description="Completion API Audio input.",
            default=None,
        ),
        ...
        if prompt or system_prompt or audio:
            if chat_completion:
                logger.warning("Mutually exclusive messages and prompt/system prompt/audio are specified. Only messages will be used.")
            else:
                messages = []  # new list
                if system_prompt:
                ...
                if audio:
                    ...
                    mm_limit = mm_config.get_limit_per_prompt("audio") if mm_config else sys.maxsize
                    user_content.extend(
                        ChatCompletionContentPartAudioParam(
                            type="audio_url",
